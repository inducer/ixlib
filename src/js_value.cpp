...
value::operatorBinary(operator_id op,ref<value> op2) const {
  if (op == OP_EQUAL) {
    ...
    return makeConstant(this == op2.get());
    }
  if (op == OP_NOT_EQUAL) {
    if (getType() == VT_NULL) 
      return makeConstant(op2->getType() != VT_NULL);
...
ref<value> 
value_with_methods::bound_method::operatorBinary(operator_id op,ref<value> op2) const {
  if  (op == OP_EQUAL) {
    bound_method *cast = dynamic_cast<bound_method *>(op2.get());
    if (cast) 
      return makeConstant((Identifier == cast->Identifier) && (Parent == cast->Parent));
    }
  if  (op == OP_NOT_EQUAL) {
    bound_method *cast = dynamic_cast<bound_method *>(op2.get());
    if (cast) 
...
// list_scope -----------------------------------------------------------------
string list_scope::stringify() const {
  string result;
  member_map::const_iterator first = MemberMap.begin(),last = MemberMap.end();
  while (first != last) {
    if (first != MemberMap.begin()) result += ", ";
    result += first->first + ":" + first->second->stringify();
    first++;
    }
  return result;
  }
...
string callable_with_parameters::stringify() const {
  return "#<callable("+concat(ParameterNameList.begin(),ParameterNameList.end(),",")+")>";
  }
...
ref<value> js_class_instance::bound_method::duplicate() {
  return this;
  }
...
ref<value> js_class_instance::bound_method::operatorBinary(operator_id op,ref<value> op2) const {
  if  (op == OP_EQUAL) {
    bound_method *cast = dynamic_cast<bound_method *>(op2.get());
    if (cast) 
      return makeConstant((Instance == cast->Instance) && (Method == cast->Method));
    }
  if  (op == OP_NOT_EQUAL) {
    bound_method *cast = dynamic_cast<bound_method *>(op2.get());
    if (cast) 
...
std::string js_class_instance::stringify() const {
  string result;
  result += "#<object ";
  if (SuperClassInstance.get())
    result += "super: "+SuperClassInstance->stringify()+" ";
  result += "("+MethodScope->stringify()+")";
  result += "("+VariableScope->stringify()+")";
  result += ">";
  return result;
  }
